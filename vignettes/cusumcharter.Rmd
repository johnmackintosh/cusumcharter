...
## A journey throught package functions
...
We input the same vector - this time using the ```cusum_single_df``` function. 
This returns :
...
plot(variance_df$x, ylim = c(-5, 15))
lines(variance_df$x)
lines(variance_df$target)
```
...
- cplus and cneg depend on the target and ```k``` values, and also relate to the current x value and previous ```cplus``` or ```cneg``` value.  
