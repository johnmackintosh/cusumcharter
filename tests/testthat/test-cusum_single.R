test_that("cusum_single works", {
  ...
  func_res <- suppressWarnings(cusum_single(test_vec))
...
test_that("target replaces mean and gives expected results", {
  ...
  func_res2 <- suppressWarnings(cusum_single(test_vec2, target = 0.16))
...
test_that("non numeric vector fails", {
  ...
  suppressWarnings(expect_error(cusum_single(test_vec3, target = 0.16)))
})
...
test_that("cusum_single warns of future deprecation", {
  test_vec <- c(0.175, 0.152, 0.15, 0.207, 0.136, 0.212, 0.166)

  expect_warning(cusum_single(test_vec))
